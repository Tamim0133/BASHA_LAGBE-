    ...
    useFocusEffect(
        React.useCallback(() => {
            const fetchAds = async () => {
                try {
                    const storedWishlist = await AsyncStorage.getItem(`wishlist_${currentUser._id}`);
                    console.log("Stored Wishlists : ", storedWishlist);
            ...
            {isLoggedIn && (
                <>
                    <ScrollView style={{ flex: 1, padding: 16 }} contentContainerStyle={{
                        ...
                        <Text style={styles.pageTitle}>Wishlist Properties</Text>
                        {items.length > 0 ? (
